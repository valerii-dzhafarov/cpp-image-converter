...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
#ifdef _MSC_VER
    if ((outfile = _wfopen(file.wstring().c_str(), L"rb")) == NULL) {
#else
    ...
    */
    ...
    std::vector<JSAMPLE> row(image.GetWidth() * 3);

    while (cinfo.next_scanline < cinfo.image_height) {
        auto* px = image.GetLine(static_cast<int>(cinfo.next_scanline)); // Color*
        for (int x = 0; x < image.GetWidth(); ++x) {
            row[3*x + 0] = (unsigned char)px[x].r; // или px[x].b, если BGR
            row[3*x + 1] = (unsigned char)px[x].g;
            row[3*x + 2] = (unsigned char)px[x].b; // без alpha
        }
